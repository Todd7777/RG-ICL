...
from datasets import get_dataset, VQA_DATASETS
...
def run_zero_shot_vqa(cfg, dataset, client, parser, output_dir):
    prompter = get_prompter("zero_shot")
    test_samples = dataset.get_test_samples()
    results = []

    for sample in tqdm(test_samples, desc="zero_shot_vqa"):
        prompt_record = prompter.build_vqa_prompt(query_sample=sample)
        inference_record = client.infer(
            messages=prompt_record.messages,
            query_id=sample.id,
            method="zero_shot",
        )
        parsed = parser.parse_vqa(
...
def run_naive_icl_vqa(cfg, dataset, client, parser, output_dir, k=6):
    prompter = get_prompter("naive_icl", k=k, seed=cfg.seed)
    ref_pool = dataset.get_reference_pool()
    test_samples = dataset.get_test_samples()
    results = []

    for sample in tqdm(test_samples, desc=f"naive_icl_vqa_k{k}"):
        prompt_record = prompter.build_vqa_prompt(
            query_sample=sample,
...
def run_rg_icl_vqa(cfg, dataset, client, parser, output_dir, metadata, global_emb,
    ...
    for sample in tqdm(test_samples, desc=f"{method}_vqa"):
...
def compute_vqa_metrics(results, metrics_engine):
    references = [r["ground_truth_answer"] for r in results]
    hypotheses = [r["parsed"]["answer"] for r in results]
    return metrics_engine.compute(references, hypotheses)
...
def main():
    ...
    parser.add_argument("--config", type=str, default="configs/experiments/vqa.yaml")
    ...
    for ds_name in dataset_names:
        ...
        if "zero_shot" in methods:
            results = run_zero_shot_vqa(cfg, dataset, client, output_parser, output_dir)
            m = compute_vqa_metrics(results, metrics_engine)
            ds_metrics["zero_shot"] = m.to_dict()

        if "naive_icl" in methods:
            results = run_naive_icl_vqa(cfg, dataset, client, output_parser, output_dir, k=k)
            m = compute_vqa_metrics(results, metrics_engine)
            ds_metrics["naive_icl"] = m.to_dict()

        if "rg_icl_global" in methods:
            results = run_rg_icl_vqa(cfg, dataset, client, output_parser, output_dir,
    ...
    print(f"\nAll VQA metrics saved to {summary_path}")
